...
| `recipient_validation_errors` | `id`, `batch_id`, `row_number`, `raw_phone`, `raw_name`, `reason`, `created_at` | CSV import errors (invalid format/duplicate) log |

### 2.4 쿠폰 및 이력
| ���̺� | �ֿ� �÷� | ���� |
| --- | --- | --- |
| `coupon_products` | `id`, `goods_id`, `name`, `face_value`, `purchase_price`, `valid_days`, `vendor_status`, `last_synced_at` | ���޻� ��ǰ ����, `goods_id` ����ũ |
| `product_sync_logs` | `id`, `sync_type`, `request_payload`, `response_code`, `synced_count`, `synced_at`, `status`, `error_detail` | COUFUN API sync ���� ����, sendCoupon ��ǰ �˻�/��� ���� 로그 |
| `coupon_issues` | `id`, `campaign_id`, `recipient_id`, `order_id`, `barcode_enc`, `valid_end_date`, `status`, `vendor_payload`, `issued_at` | ���� �߱� ���, ���ڵ� ��ȣȭ ���� |
| `coupon_status_history` | `id`, `coupon_issue_id`, `status`, `status_source`, `status_at`, `memo` | ���޻� push/poll ���, CS ó�� �̷� |
| `coupon_exchange_details` | `id`, `coupon_issue_id`, `exchange_store`, `exchange_at`, `cancel_at`, `remain_amount` | COUFUN ��ȯ ���� API ���� |
### 2.5 발송/결과
| ���̺� | �ֿ� �÷� | ���� |
| --- | --- | --- |
| `mms_jobs` | `id`, `campaign_id`, `recipient_id`, `client_key`, `ums_msg_id`, `req_date`, `status`, `retry_count` | SNAP Agent INSERT ��� ���� |
| `dispatch_results` | `id`, `mms_job_id`, `done_code`, `done_desc`, `telco`, `sent_at`, `completed_at` | UMS_LOG ���� ��� |
| `coupon_templates` | `id`, `template_key`, `name`, `editor_schema`, `default_variables`, `status`, `updated_at` | sendCoupon �޽��� ���ø��� ����/��ä |
| `media_assets` | `id`, `file_name`, `storage_path`, `mime_type`, `width`, `height`, `checksum`, `uploaded_by`, `uploaded_at` | ���� ��Ź/�̹��� ��ü ��� |
| `rendered_mms_assets` | `id`, `campaign_id`, `recipient_id`, `template_id`, `asset_id`, `file_path`, `file_hash`, `created_at` | template + recipient �� ���κ��� ��ȯ�� MMS �̹��� |
### 2.6 CS 및 감사
| 테이블 | 주요 컬럼 | 설명 |
| --- | --- | --- |
...
| ���̺� | �ֿ� �÷� | ���� |
| --- | --- | --- |
| `report_exports` | `id`, `report_type`, `filter_payload`, `row_count`, `status`, `requested_by`, `requested_at`, `completed_at`, `file_path` | sendQuery �˻� ��� ť �� �ٿ�ε� ������ ���� ���� |
| `query_cursor_bookmarks` | `id`, `query_type`, `last_id`, `filters_hash`, `expires_at`, `created_by` | cursor 기반 API ������ last_id ����/���� ��鼭 ���� |

### 2.8 ���� SNAP/COUFUN �α� ����
| ��ü | �ֿ� �÷� | ���� |
| --- | --- | --- |
| `snap_msg_view` | `ums_msg_id`, `client_key`, `req_ch`, `msg_status`, `telco`, `done_code`, `done_desc`, `synced_at` | SNAP Agent `UMS_MSG`/`UMS_LOG` mirror view, sendCs ��Ÿ SNAP ȣ��� ��ġ ��� |
| `coufun_status_cache` | `coupon_issue_id`, `status`, `status_source`, `status_at`, `synced_at`, `raw_payload` | COUFUN status/poll ������ �ð� ���� cache ��, sendQuery/CS ó�� ���� |
...
## 6. �ε��� �� ���� ����
- `campaign_recipients`: `(campaign_id, status)`, `(enc_phone_hash)` (SHA-256 �ؽ� �÷�) �ε���.
- `coupon_issues`: `(campaign_id, status)`, `(order_id)`, `(valid_end_date)` + sendQuery ������ `(campaign_id, issued_at DESC)` covering index �� ����.
- `dispatch_results`: `(done_code)`, `(completed_at)` �ε����� ��� ���� ����ȭ.
- `report_exports`: `(report_type, status, requested_at)` �ε��� + `requested_by` filter�� ���߿� ���� ť ��쿡 ���.
- `recipient_validation_errors`: `(batch_id, row_number)` index for CSV error review and UI download.
- `rendered_mms_assets`: `(campaign_id, recipient_id)` �ε��� + `file_hash` unique index�� download ��� ��ȿ��.
- ��Ƽ�Ŵ�: `dispatch_results`, `coupon_status_history`, `report_exports`�� �� ���� ��Ƽ�� ����.
- ��ŷ/����: `coupon_issues`, `coupon_status_history`�� YYYYMM ��ŷ(�մ� 1 ��ϴ�)�� sendQuery range scan �� ���̿���.
## 7. 데이터 보존 정책
